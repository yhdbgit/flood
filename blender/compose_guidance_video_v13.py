"""Compose the V13 official-map 60-second story, overlays, final card, and TTS in Blender VSE."""
...
BLEND_PATH = ROOT / "blender" / "osong_guidance_v13.blend"
...
def main():
    ...
    for segment in manifest["segments"]:
        ...
        sound_strip = editor.strips.new_sound(
            f"TTS_{segment['id']}", segment["audio_path"], 4, segment["start_frame"]
        )
        sound_strip.frame_final_duration = duration
        sound_strips.append(sound_strip)
    ...
    scene["official_floodmap_scenario"] = "국가하천 금강권역 100년 빈도"
    ...
    if not video.is_file() or video.stat().st_size < 100_000 or b"ftyp" not in video.read_bytes()[:32]:
        raise RuntimeError("V13 final MP4 render failed")
    report = {
        "status": "ok",
        "scene": scene.name,
        "movie_strips": 1,
    ...
    print("[guidance-v13] composition complete")
