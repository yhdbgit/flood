...
현재 저장소 기준으로 **V18 통합 컨텍스트와 4개 LangGraph 에이전트 영상 제작 연결**까지 구현됐다. V18은 기상청 05시 단기예보, 공식 홍수위험범위, 등록 농경지, 행동요령과 대피시설을 하나의 컨텍스트로 묶고, 발동 조건을 통과한 경우에만 1~2분 안내 영상 워크플로를 실행한다.
