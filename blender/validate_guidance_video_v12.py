"""Validate the saved V12 60-second VSE project and final MP4."""
...
VIDEO_PATH = OUT / "osong_guidance_v12_60s.mp4"
...
if scene.name != "Osong_Guidance_V12": failures.append(f"scene invalid: {scene.name}")
...
markers = [value for value in ("OPENAI_API_KEY", "HRFCO_API_KEY", "sk-") if value in text_dump]
legacy_markers = [value for value in ("Gangnae", "GANGNAE", "강내면") if value in text_dump]
if markers: failures.append(f"secret markers found: {markers}")
if legacy_markers: failures.append(f"legacy markers found: {legacy_markers}")
report = {
