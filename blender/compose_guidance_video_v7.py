...
MANIFEST_PATH = ROOT / "output" / "guidance_v7" / "guidance_manifest.json"
BLEND_PATH = ROOT / "blender" / "gangnae_guidance_v7.blend"
REPORT_PATH = ROOT / "output" / "guidance_v7" / "composition_report.json"
...
def main():
    ...
    scene.name = "Gangnae_Guidance_V7"
    ...
    if not video.is_file() or video.stat().st_size < 100_000 or b"ftyp" not in video.read_bytes()[:32]:
        raise RuntimeError("V7 MP4 render failed")
    report = {
        "status": "ok",
        "scene": scene.name,
    ...
    print("[guidance-v7] composition complete")
