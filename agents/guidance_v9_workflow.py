"""V9 60-second flood guidance workflow aligned to the repeated camera story."""
...
SOURCE_BLEND = ROOT / "blender" / "gangnae_inundation_v5.blend"
BASE_BUILDER = ROOT / "blender" / "build_gangnae_story_v9.py"
COMPOSER = ROOT / "blender" / "compose_guidance_video_v9.py"
BLENDER = Path("/Applications/Blender.app/Contents/MacOS/Blender")
OUT = ROOT / "output" / "guidance_v9"
ASSETS = OUT / "visuals"
AUDIO = OUT / "audio"
BASE_VIDEO = OUT / "gangnae_story_v9_base.mp4"
FINAL_VIDEO = OUT / "gangnae_guidance_v9_60s.mp4"
...
def context_agent(state: StoryState) -> Dict[str, Any]:
    ...
    for path in (SOURCE_BLEND, BASE_BUILDER, COMPOSER):
        if not path.is_file():
    ...
    segments = [
        {
            ...
            "narration": f"강내면에 {forecast['rain_start_label']}부터 많은 비가 예상됩니다. 24시간 예상 강수량은 {forecast['predicted_24h_rain_mm']} 밀리미터이며 하천 주변 저지대부터 물이 퍼집니다.",
            "visual_type": "overlay",
        },
        {
            ...
            "narration": "하천과 가까운 등록 농경지를 확대해 확인합니다.",
            "visual_type": "overlay",
        },
        {
            ...
            "narration": f"하천 수위가 오르며 물이 농경지 방향으로 퍼집니다. 경계 단계에는 등록 농경지의 약 {warning:.1f} 퍼센트가 영향을 받습니다.",
            "visual_type": "overlay",
        },
        {
            "id": "wide_flood_2", "start_frame": 577, "end_frame": 816,
            "title": "침수 확산 경로 재확인",
            "subtitle": "하천 주변 → 저지대 → 등록 농경지",
            "narration": "전체 침수 과정을 다시 확인합니다. 하천 주변에서 시작된 물이 낮은 지형을 따라 등록 농경지까지 확산됩니다. 침수 범위가 넓어지는 순서를 다시 확인하십시오.",
            "visual_type": "overlay",
        },
        {
        ...
        },
        {
            ...
            "title": "심각 단계 농경지 영향",
            "subtitle": f"등록 농경지 {serious:.1f}% 영향 예상",
            "narration": f"심각 단계에는 등록 농경지의 {serious:.1f} 퍼센트가 영향을 받습니다. 비가 오기 전에 배수로를 확인하고 농기계와 농자재를 안전한 곳으로 옮기십시오. 비닐하우스 결박 상태도 확인하십시오.",
            "visual_type": "overlay",
        },
        {
...
def safety_agent(state: StoryState) -> Dict[str, Any]:
    ...
    required = ["80", "62.5", "100.0", "배수로", "농기계", "비닐하우스", "논둑이나 물꼬", "일일구"]
    forbidden = ["시뮬레이션", "실제 상황이 아닙니다", "증거가 되지 않습니다", "공모전 시연용", "공식 경보입니다", "즉시 대피하십시오"]
    ...
    passed = not missing and not found and timeline_valid
    review = {
        "passed": passed,
    ...
    if not passed:
        raise RuntimeError(f"V9 safety review failed: {review}")
...
def final_slide_asset(state: StoryState, item: Dict[str, Any], path: Path):
    ...
    draw.text((72, 477), "기상청·지자체 안내 확인  |  AI 생성 음성 사용", font=font(17), fill="#A8C4D0")
    image.save(path)
...
def tts_agent(state: StoryState) -> Dict[str, Any]:
    ...
    base_speed = float(os.getenv("OPENAI_TTS_SPEED_V9", "1.15"))
...
def build_graph():
    ...
    nodes = [
        ("context_agent", context_agent), ("script_agent", script_agent), ("safety_agent", safety_agent),
...
def main():
    ...
    result = graph.invoke({"run_id": "GANGNAE-GUIDANCE-V9-60S", "trace": []})
    print(json.dumps({
