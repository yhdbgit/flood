"""Build and optionally render the official-map-constrained 47-second Osong V13 base story."""
...
BLEND_PATH = ROOT / "blender" / "osong_story_v13_base.blend"
...
def make_flood_material(base, step):
    ...
    return base.new_material(
        f"V13_Mat_OfficialInundation_{step:02d}",
        colour,
        roughness=0.20,
        metallic=0.05,
...
def build_flood_objects(base, context, inundation):
    collection = ensure_collection("V13_OFFICIAL_INUNDATION")
    ...
    for item in inundation["progression"]:
        ...
        for index, polygon in enumerate(item["polygons"]):
            obj = base.polygon_mesh(
                f"OfficialInundationV13_S{step:02d}_P{index:02d}",
                polygon["exterior"],
                sampler,
                collection,
                material,
...
def rebuild_story_camera(scene):
    ...
    data.name = "Camera_Osong_Story_V13_Data"
    data.clip_start = 0.1
    data.clip_end = 100_000.0
    camera = bpy.data.objects.new("Camera_Osong_Story_V13", data)
...
def configure_render(scene):
    scene.name = "Osong_Official_Story_V13_Base"
    scene.frame_start = 1
...
def main():
    ...
    scene["story_version"] = "V13"
    scene["field_id"] = inundation["field"]["field_id"]
    scene["official_scenario"] = inundation["source"]["dataset"]
    scene["story_timeline"] = "20s overview official flood, dry zoom, 20s field flood, hold, shelter map in VSE"
    scene["flood_progression_steps"] = 10
    scene["flood_step_seconds"] = 2.0
    scene["base_duration_seconds"] = 47.0
    ...
    if not args.setup_only:
        ...
        if not BASE_VIDEO.is_file() or BASE_VIDEO.stat().st_size < 100_000:
            raise RuntimeError("V13 base MP4 render failed")
        if b"ftyp" not in BASE_VIDEO.read_bytes()[:32]:
            raise RuntimeError("V13 base MP4 container is invalid")
