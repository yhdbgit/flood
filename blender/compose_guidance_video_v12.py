"""Compose the V12 60-second story, overlays, final card, and TTS in Blender VSE."""
...
BLEND_PATH = ROOT / "blender" / "osong_guidance_v12.blend"
...
def main():
    ...
    if not video.is_file() or video.stat().st_size < 100_000 or b"ftyp" not in video.read_bytes()[:32]:
        raise RuntimeError("V12 final MP4 render failed")
    report = {
        "status": "ok",
        "scene": scene.name,
        "movie_strips": 1,
    ...
    print("[guidance-v12] composition complete")
