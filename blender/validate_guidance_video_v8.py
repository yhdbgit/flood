"""Validate the saved V8 VSE project and final guidance MP4."""
...
VIDEO_PATH = OUT / "gangnae_guidance_v8.mp4"
...
if scene.name != "Gangnae_Guidance_V8":
    failures.append(f"scene name invalid: {scene.name}")
if scene.get("guidance_mode") != "mvp_demo_not_live_forecast":
    failures.append("demo mode label is missing")
if not scene.get("ai_voice_disclosure"):
    failures.append("AI voice disclosure is missing")
if manifest.get("schema_version") != "2.0":
    failures.append("V8 manifest schema version is missing")
if manifest.get("generation_meta", {}).get("provider") not in {"openai", "rule_fallback"}:
    failures.append("script provider metadata is invalid")
if sum(manifest.get("tts_meta", {}).get("provider_counts", {}).values()) != len(manifest["segments"]):
    failures.append("TTS provider metadata is invalid")

text_dump = "\n".join(str(value) for value in [scene.name, *[scene.get(key) for key in scene.keys()]])
api_key_markers = [marker for marker in ("1FB4A0FE", "HRFCO_API_KEY", "OPENAI_API_KEY", "sk-") if marker in text_dump]
if api_key_markers:
